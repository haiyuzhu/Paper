...
这些方法的提出旨在以较低的成本更准确、更加快速的获取人体的三维模型。
...
然后将这些模型结合人体姿态信息从而训练出这个人的参数化三维人体模型，如图~\ref{fig:DepthSensor_Zhang}~中(b)。
...
最后再保证最大颜色连续性的前提下将二维高斯函数和三维高斯函数一起做优化得到模型更多的细节。
\begin{figure}[H]
  \centering
  % Requires \usepackage{graphicx}
...
多个相机使用不但重建系统的成本没有降低，还使得设备环境搭建比较繁琐。
...
单视角重建相比多视角重建而言它可以获取到的信息就更少，所以大多重建方法都会先建立参数化人体模型，然后在参数化人体模型的基础上利用人体的轮廓信息作为约束求解重建问题~\cite{guan2009estimating,hasler2010multilinear,jain2010moviereshape,zhou2010parametric}。
...
然后使用拉普拉斯变换将双线性模型的体型跟图像上的人体进行融合得到图~\ref{fig:single_view_Hasler}(b)。
接着将人体的姿态或者体型跟图形进行融合得到图~\ref{fig:single_view_Hasler}(c)。
...
他们将参数化的三维模型投影到图片中的人体轮廓上，利用KLT（Kanade-Lucas-Tomasi）算法跟踪图像上手动标定的特征点，借此同时优化出图片中人物的三维体型和姿态。
...
再利用调整人体模型之后可以找到更多对应点（图~\ref{fig:single_view_Zhou}(c)人体下半身）优化人体姿态和体型，图~\ref{fig:single_view_Zhou}(d)。
\begin{figure}[H]
  \centering
  % Requires \usepackage{graphicx}
...
本文提出的方法能够借助一个普通的RGB相机自动的重建出一个动态的三维人体模型，如图~\ref{fig:introduction}。
...
本文研究的重点研究的问题是如何利用一个普通的~RGB~相机拍摄人体运动的视频，在没有人为添加约束的前提下自动重建出高质量的人三维模型。
为了验证方法的有效性和稳定性，我们建立一个数据集，将我们的方法跟别的方法进行对比验证。
本文的章节组织如下：
...
通过公开的数据集我们训练出自己的~SCAPE~模型，然后借助人体姿态检测获取每帧图像人体的三维骨架信息。
结合人体运动信息，我们对图像上的人体进行运动学分类将非刚性重建问题转化成刚性重建。
利用稠密光流我们获取到同一身体部位上点的对应关系，结合稀疏光束平差法我们获得不同身体部位不同时刻的三维模型和~SCAPE~模型融合得到许多实例。
