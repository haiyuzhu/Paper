...
在过去的几十年中，学者们探索出了许多种方法来获取人体的三维模型，例如利用三维扫描仪获取三维人体模型的~\cite{anguelov2005scape,werghi2007segmentation}，借助深度传感器获取深度图来重建三维人体模型~\cite{newcombe2011kinectfusion,Izadi2011KinectFusion, weiss2011home,tong2012scanning,li20133d,zeng2013templateless,zhang2014quality,newcombe2015dynamicfusion,dou20153d}，利用多视角重建完成三维人体重建的~\cite{deaguiar2008performance,gall2009motion,liu2013markerless,huang2014human,robertini2014efficient,zollhofer2014real}。
...
Kinect~可以提供最大帧率~30fps~的深度图和彩色图，它对环境的适应能力强、精度较高、价格低廉，性价比很高。
...
基于~Kinect~的三维人体重建或多都借鉴了~KinectFusion~的思路，他们大多利用某时刻人体局部的三维模型来恢复人体完整的三维模型。
但是从局部恢复整体的问题本身就是一个欠约束的问题，所以为了解决这样一个欠约束的问题，目前的解决方法主要有基人体于模板的方法，如~\cite{weiss2011home,tong2012scanning,li20133d,zhang2014quality}。
...
所以他们就将某一视角下的人体模型作为模板，并将其他视角下的人体模型用刚性、非刚性融合算法将模型融合到模板中，最后利用融合过程中得到的对应关系做全局的优化得到完整的人体模型。
...
（1）、Kinect获取的每帧深度信息都用模型到部分(Model-to-part Method)的方法将“图”融合到深度信息上。
...
Bogo等人~\cite{bogodetailed}~扩展了~SCAPE~模型提出了一个名为“Delta”的多分辨率人体模型。从单目的RGB-D图像序列中恢复出动态人体的体型参数之后，结合高分辨率位移图获取人体表面的细节，从而得到精细的三维人体模型。
...
而~Theobalt~等人~\cite{deaguiar2008performance,Theobalt2010Performance}~抛弃了传统用骨骼驱动或将运动参数化的方法，而是用捕捉到的模型变化来驱动人体模型。
...
Zollhofer等人~\cite{zollhofer2014real}~利用双目相机模拟深度相机工作获取场景的深度信息，结合体融合方法和非刚性三维重建算法重实时地重建出人体的三维模型。
...
将前一帧的人体模型进行变形，在拟合图像上的人体的同时优化化图（c）中的人体骨架。
...
最后将当前帧的人体轮廓结合和上一帧的人体姿态、体型结合得到当前帧的人体姿态和体型，图（e，f）。
\begin{figure}[H]
  \centering
  % Requires \usepackage{graphicx}
...
Robertini~等人~\cite{robertini2014efficient}~在Gall等人~\cite{gall2009motion}方法的基础上提出了用三维高斯函数来表示人体体型的新方法，并将三维人体重建问题转化成连续图像序列中人体表面顶点全局优化的问题，从而能够获取模型表面更多的细节使得模型更为精细。
...
\begin{figure}[H]
  \centering
  % Requires \usepackage{graphicx}
  \includegraphics[width=0.88\textwidth]{figures/multi_view_Robertini.jpg}
...
虽然深度相机重建出来的模型精度较高，并且已经能够应用在动态的场景，但是，深度相机体积较大便携性很差，高质量深度相机价格高昂等因素限制了深度相机重建方案的普及。
多相机三维重建不仅需要对多个相机的图像进行帧同步，还要对相机的位置进行标定。
且多个相机使用不但重建系统的成本没有降低，还使得设备环境搭建比较繁琐。
...
但是~Yu~等人~\cite{yu2015direct}~虽然没有用参数化的模型做重建但是他们也需要先用~SFM(Structure From Motion)~的方法得到目标初始的三维模型。
此外，Guan~等人~\cite{guan2009estimating}~还使用从阴影到结构的方法提高重建模型的质量，消除人体遮挡带来的影响。
但是，这也是的方法受限于光源的方向和强度，并且在多个光源的情况下会使得重建问题变得十分复杂。
总的来说，单视角由于投影变换损失的信息过多，且现实中不可避免的遮挡问题，上述方法或多或少都需要人为的添加一些约束而无法做到自动化单视角三维人体重建。

Guan等人~\cite{guan2009estimating}~的方法如图~\ref{fig:single_view_Guan}~所示。
他们首先计算出单幅图片上的人体姿态、光源方向、人体体型以及前景和背景的分割，并利用公开的人体模型数据集训练出~SCAPE~模型，。
接着利用人体的轮廓、边缘和阴影信息，将一个静态人体模型拟合到图像上得到人体的体型信息，最后得到的三维模型可以动画等领域。
由于他们充分利用了图像中的阴影、边缘信息，所以他们的方法可以应对人体有遮挡的情况。
尽管如此，他们的方法在人体姿态估计时需要手动在图像上标注出人体关节点作为初值。
\begin{figure}[H]
  \centering
  % Requires \usepackage{graphicx}
  \includegraphics[width=0.9\textwidth]{figures/single_view_Guan.jpg}
...
在使用拉普拉斯变换将双线性模型的体型跟图像上的人体进行融合得到图~\ref{fig:single_view_Hasler}(b)。
接着将人体的姿态或者体型跟图像进行融合得到图~\ref{fig:single_view_Hasler}(c)。
最后迭代将人体姿态、体型跟图像上人体轮廓进行融合，使得模型尽可能跟图像上人体轮廓融合~\ref{fig:single_view_Hasler}(d)，最终得到人体模型~\ref{fig:single_view_Hasler}(e)。
虽然多张图片可以提高他们模型的精度，但是他们也需要手动标注一些特征，并且重建出来模型的表面依旧有许多的缺陷。
\begin{figure}[H]
  \centering
  % Requires \usepackage{graphicx}
  \includegraphics[width=\textwidth]{figures/single_view_Hasler.jpg}
...
他们将参数化的三维模型投影到图片中的人体轮廓上，利用KLT（Kanade-Lucas-Tomasi）算法跟踪图像上手动标定的特征点，与此同时优化出图片中人物的体型和姿态。
他们的参数模型是有别于~SCAPE~模型的，他们没有训练~SCAPE~中每个三角面片因姿态变换而引起的变换矩阵，而是直接将模型和人体骨架进行融合绑定，因为在对图像上人体参数进行修改时是不需要这样的细节的。
\begin{figure}[H]
  \centering
  % Requires \usepackage{graphicx}
  \includegraphics[width=\textwidth]{figures/single_view_Jain.jpg}
...
他们首先利用用户手动标注的关节点估计出三维人体姿态，图~\ref{fig:single_view_Zhou}(a)。
接着找出模型轮廓和人体轮廓的对应点，图~\ref{fig:single_view_Zhou}(b)。
利用人体上半身的对应点先对人体的体型、姿态进行估计并调整人体模型，图~\ref{fig:single_view_Zhou}(c)。
再利用调整人体模型之后新的对应点（图~\ref{fig:single_view_Zhou}(c)人体下半身）优化人体姿态和体型，图~\ref{fig:single_view_Zhou}(d)。
\begin{figure}[H]
  \centering
  % Requires \usepackage{graphicx}
  \includegraphics[width=\textwidth]{figures/single_view_Zhou.jpg}
...
他们主要是用~SFM~重建时得到模型每个点的颜色信息，将非刚性实时重建转化成优化问题。

\section{本文主要工作}

由于人体具有较高的灵活性，单个相机获取的获取的信息十分有限，因此自动化的动态人体的单相机三维重建一直是一个充满挑战的问题。
本文提出的方法能够借助一个普通的~RGB~相机自动的重建出一个动态的三维人体模型，如图~\ref{fig:introduction}。
图中第一行是我们通过相机获取人体运动的序列，第二行是我们重建出来的每一帧中人体的三维模型。
\begin{figure}[ht]
  \centering
  % Requires \usepackage{graphicx}
...
因为单相机图像的获取其实是对空间物体做投影变换，再加上物体本身的遮挡会使得物体很大一部分信息丢失。
但如果物体能够运动起来，我们就可以在某个时间段捕获到之前遮挡掉的一部分信息，借助这些信息我们就可以提高人体模型的重建精度。
...
\begin{itemize}
  ...
  \item 率先提出将人体的运动作为分类图像上人体部位的重要依据，并在此基础上提出一个运动学分类器，结合人体在图像上的运动，将图像上人体部位进行分类，具有较高的精度；
...
本文研究的重点研究的问题是如何利用一个普通的~RGB~相机通过拍摄人体运动的视频，自动重建出高质量的人三维模型。
...
最后分析了我们算法的时间复杂度，找出我们方法的效率瓶颈，找出系统可能的改进方向。
...
在这个章节中，我们将详细地介绍图~\ref{fig:pipeline}~种展示的动态三维人体重建系统。
系统整体上可以分为以下几个主要步骤：

\begin{enumerate}
  \item 训练数据集，获取 SCAPE 模型。
  \item 我们使用一个软件检测子来获取图像上人体~2D、~3D的关节点位置，而不需要去手动标定图像上人体的关节点位置。
  \item 利用人体的运动学信息，我们优化了获取到的三维人体姿态信息，并将图像上的人体划分成许多刚性块的组合，使得动态三维人体重建从一个非刚性重建问题转换成刚性重建问题。
...
模型的体型变换是跟人体的姿态没有关系的，所以我们将不同体型的人通过姿态变换将他们驱动到同一姿态下，并且为了描述人体体型变化我们引入了一个新的线性变换矩阵集~$S^i_k$~来表示第~i~个实例上第~k~的三角面片的形变。
并且我们假设实例~i~上的三角面片~$p_k$~的形变是先通过姿态变换~$Q^i_k$~，然后再进行人体体型变换~$S^i_k$~，最后再进行对应身体部位的旋转变换~$R^i_{B_k}$~，具体可用公式~\ref{eqn:scape_shape}~表示。
...
\end{equation}
我们可以通过~PCA(Principal Component Analysis)~求解出参数~$U,\mu$~以及系数~$\beta^i$~。

但是这里我们还不知道每个三角面片的体型变换~$S^i_k$~，所以，我们利用数据集中不同体型的人计算出~$S^i_k$~。
跟上面的思路一样，我们求解目标函数~\eqref{eqn:scape_shape}就可以得到~$S^i$~。
\begin{equation}\label{eqn:scape_shape}
  \min_{S^i}\sum_{k}\sum_{j=2,3} \left\| R^i_k S^i_k Q^i_k \hat{v}_{k,j} - v^i_{k,j} \right\|^2 + w_s \sum_{k_1,k_2 \  adj} \left\| S^i_{k_1} - S^i_{k-2}\right\|^2
\end{equation}
由于这里位置了只有~$S^i_k$~，所以我们可以直接使用最小二乘求解。
...
我们假设 $P_i$ 表示第 $i$ 帧图片中人体的三维姿态，由于相邻帧中人体姿态具有较高的相关性，所以我们可以认为 $P_i$ 是由第 $i-1$ 帧上三维人体姿态 $P_i-1$ 变换而来。
这样一种动态关系我们可以用公式表示为：
...
首先，我们利用之前从图片中获得的三维人体姿态将 SCAPE 模型驱动到当前图片中人体的姿态下，然后用 Z-buffer 算法获取该姿态下相机所能获取到的 SCAPE 模型的前景部分，再将前景部分不同的身体部位跟它对应的稠密三维重建结果借助 CPD 算法进行非刚性融合，融合之后我们还获得了 SCAPE 模型和稠密重建模型上的点对应关系。
...
最后我们将模型驱动到不同帧中人体对应的姿态下，在加入当前帧中人体运动的非刚性成分之后就得到拥有细节的三维人体模型，结果如图中最后一行所示。
...
尽管目前我们的系统还是比较耗时的，但是它的确是第一个通过普通 RGB 相机自动重建出动态三维人体模型的系统。
...
\begin{itemize}
  ...
  \item 分离出了人体运动中的“刚性成分”和“非刚性成分”，应用场景广阔。分析人体模型中的刚性成分我们可以得到人体模型的体型参数，可以用于虚拟试衣等应用场景，人体模型的非刚性成分可以用于获得人体不同姿态下的精细模型，可以用于计算机动画等应用场景；
